package com.aniket.cartservice.controller;
...
import com.aniket.cartservice.model.Cart;
import com.aniket.cartservice.model.IdGenerator;
import com.aniket.cartservice.model.Item;
import com.aniket.cartservice.model.Product;
import com.aniket.cartservice.repository.CartRepository;
import com.aniket.cartservice.repository.IdRepository;
...
public class CartController {
	...
	@GetMapping("/{userId}") // get the details of particular user cart details  
	...
	@GetMapping("/items/{userId}") // product details and items details
	public Collection<Item> getItems(@PathVariable("userId") int userId){
		return getCart(userId).getItems().values();
	}
	
	@PostMapping("/create/{userId}") // to create the cart for particular user id
	...
	public Cart addItem(@PathVariable("prodId") int prodId, @PathVariable("userId") int userId) {
		...
		if(items.containsKey(prodId)) {
			i.setQuantity(i.getQuantity()+1); // add cart 
	...
	public Cart removeItem(@PathVariable("prodId") int prodId, @PathVariable("userId") int userId) {
		...
		Item i=items.get(prodId);
		if(i.getQuantity()>1) {
			i.setQuantity(i.getQuantity()-1); // remove method
			i.setPrice(product.getPrice()*i.getQuantity());
			items.put(prodId, i);
		}
		else {
		...
		cart.setItems(items);
		cart.setTotal(0);
		items.values().forEach(item -> cart.setTotal(cart.getTotal()+item.getPrice())); // for getting all the total price in cart
		cartRepo.save(cart);
		return cart;
	}
	
	@GetMapping("/cartItems/{userId}") // total no. of quantity of items inside the cart
